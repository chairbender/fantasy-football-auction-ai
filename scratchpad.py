...
def do_cem():
    ...
    cem = CEMAgent(model=model, nb_actions=nb_actions, memory=memory, nb_steps_warmup=0,
                   batch_size=1, train_interval=1, elite_frac=0.05)
    cem.compile()
    ...
    # Ctrl + C.
    cem.fit(env, nb_steps=100000, visualize=False, verbose=1, callbacks=[ObservationChecker()])
    # After training is done, we save the best weights.
    cem.save_weights('cem_{}_params.h5f'.format(ENV_NAME), overwrite=True)
    # Finally, evaluate our algorithm for 5 episodes.
    cem.test(env, nb_episodes=5, visualize=False, callbacks=[Checker()])
...
def do_dqn():
    ...
    dqn = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, nb_steps_warmup=1000,enable_dueling_network=True,
                   target_model_update=1e-2, policy=policy, batch_size=64, train_interval=32)
    dqn.compile(Adam(lr=1e-3), metrics=['mae'])

    #dqn.load_weights('dqn_{}_params.h5f'.format(ENV_NAME))
    ...
    log = LogTesting(100)
    #for i in range(100):
    dqn.fit(env, nb_steps=50000, verbose=1, visualize=False, callbacks=[log])
